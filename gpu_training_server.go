...
func NewModel(gpuConfig *GPUConfig) *Model {
    ...
    // Xavier/Glorot initialization with gain=5.0 for more exploration
    for i := range m.actorWeights {
        if i < 6*64 {  // input->hidden1
            m.actorWeights[i] = float32(rand.NormFloat64()) * float32(math.Sqrt(10.0/float64(6)))  // gain=5.0
        } else if i < 6*64 + 64*64 {  // hidden1->hidden2
            m.actorWeights[i] = float32(rand.NormFloat64()) * float32(math.Sqrt(10.0/float64(64)))
        } else {  // hidden2->output
            m.actorWeights[i] = float32(rand.NormFloat64()) * float32(math.Sqrt(10.0/float64(64)))
        }
    }
    ...
    for i := range m.criticWeights {
        m.criticWeights[i] = float32(rand.NormFloat64()) * float32(math.Sqrt(10.0/float64(64)))
    }

    if gpuConfig != nil && gpuConfig.IsAvailable {
        // Initialize CUDA context and resources
        if err := m.initializeCUDA(gpuConfig); err != nil {
            log.Printf("Warning: Failed to initialize CUDA: %v", err)
            m.device = "cpu"
        }
    } else {
        m.device = "cpu"
...
func (m *Model) Forward(obs []float32) ([]float32, float32) {
    ...
    if m.device == "cuda" {
        ...
        if err := result(C.cuMemcpyHtoDAsync(m.gpuBuffers.input, unsafe.Pointer(&obs[0]), C.size_t(len(obs)*4), m.cudaStreams.memory)); err != nil {
            log.Printf("Warning: Failed to copy input to GPU: %v", err)
...
func (m *Model) forwardCPU(obs []float32) ([]float32, float32) {
    ...
    for i := 0; i < 64; i++ {
        ...
        hidden1[i] = float32(math.Tanh(float64(sum)))
    }
    ...
    for i := 0; i < 64; i++ {
        var sum float32
        for j := 0; j < 64; j++ {
            sum += hidden1[j] * m.actorWeights[6*64+j*64+i]
        }
        hidden2[i] = float32(math.Tanh(float64(sum)))
    }
    ...
    for i := 0; i < 3; i++ {
        var sum float32
        for j := 0; j < 64; j++ {
            sum += hidden2[j] * m.actorWeights[6*64+64*64+j*3+i]
        }
        actions[i] = sum
    }
...
func (m *Model) Train(batch []Experience) {
    ...
    if m.device == "cuda" {
        ...
        if err := makeContextCurrent(m.cudaContext); err != nil {
            log.Printf("Warning: Failed to set CUDA context in Train: %v", err)
            contextMutex.Unlock()
            return
        }
        contextMutex.Unlock()
        
        log.Printf("[mario] Performing GPU training step with batch size: %d", len(batch))
        ...
        if err := result(C.cuMemAlloc(&actorGradients, C.size_t(len(m.actorWeights)*4))); err != nil {
            log.Printf("Warning: Failed to allocate GPU memory for actor gradients: %v", err)
            return
        }
        defer C.cuMemFree(actorGradients)

        if err := result(C.cuMemAlloc(&criticGradients, C.size_t(len(m.criticWeights)*4))); err != nil {
            log.Printf("Warning: Failed to allocate GPU memory for critic gradients: %v", err)
            return
        }
        ...
        if err := result(C.cuMemcpyHtoD(actorGradients, unsafe.Pointer(&zeroActorGradients[0]), C.size_t(len(m.actorWeights)*4))); err != nil {
            log.Printf("Warning: Failed to initialize actor gradients: %v", err)
            return
        }
        if err := result(C.cuMemcpyHtoD(criticGradients, unsafe.Pointer(&zeroCriticGradients[0]), C.size_t(len(m.criticWeights)*4))); err != nil {
            log.Printf("Warning: Failed to initialize critic gradients: %v", err)
            return
        }
        ...
        for _, exp := range batch {
            ...
            for i := range action {
                policyLoss += float32(math.Log(float64(action[i]))) * float32(exp.Action[i]) * advantage
            }
            policyLoss = -policyLoss / float32(len(batch))

            // Calculate actor gradients
            actorGrads := make([]float32, len(m.actorWeights))
            for i := range action {
                // Calculate gradients for action probabilities
                grad := float32(exp.Action[i]) * advantage
                // Apply gradient to corresponding weights
                for j := 0; j < 256; j++ { // hidden size
                    actorGrads[i*256+j] = grad * exp.Observation[j%6] // input size is 6
                }
            }

            // Copy actor gradients to GPU
            if err := result(C.cuMemcpyHtoD(actorGradients, unsafe.Pointer(&actorGrads[0]), C.size_t(len(actorGrads)*4))); err != nil {
                log.Printf("Warning: Failed to copy actor gradients to GPU: %v", err)
                return
            }

            // Calculate critic gradients
            criticGrad := value - returns  // Negative gradient for value prediction
            if err := result(C.cuMemcpyHtoD(criticGradients, unsafe.Pointer(&criticGrad), C.size_t(4))); err != nil {
                log.Printf("Warning: Failed to copy critic gradient to GPU: %v", err)
                return
            }
            ...
            if err := result(C.cuLaunchKernel(
                m.kernels.train,
                C.uint((len(m.actorWeights)+255)/256), 1, 1, // grid dimensions
                256, 1, 1, // block dimensions
                0, m.cudaStreams.compute, // shared memory and stream
                &args[0], nil, // kernel arguments
            )); err != nil {
                log.Printf("Warning: Failed to launch actor training kernel: %v", err)
                return
            }
            ...
            if err := result(C.cuLaunchKernel(
                m.kernels.train,
                C.uint((len(m.criticWeights)+255)/256), 1, 1, // grid dimensions
                256, 1, 1, // block dimensions
                0, m.cudaStreams.compute, // shared memory and stream
                &args[0], nil, // kernel arguments
            )); err != nil {
                log.Printf("Warning: Failed to launch critic training kernel: %v", err)
                return
            }
        }
        ...
        if err := result(C.cuMemcpyDtoH(unsafe.Pointer(&m.actorWeights[0]), m.gpuBuffers.actorWeights, C.size_t(len(m.actorWeights)*4))); err != nil {
            log.Printf("Warning: Failed to copy actor weights from GPU: %v", err)
            return
        }
        if err := result(C.cuMemcpyDtoH(unsafe.Pointer(&m.criticWeights[0]), m.gpuBuffers.criticWeights, C.size_t(len(m.criticWeights)*4))); err != nil {
            log.Printf("Warning: Failed to copy critic weights from GPU: %v", err)
            return
        }
    } else {
        log.Printf("[mario] Performing CPU training step with batch size: %d", len(batch))
        // CPU implementation
        for _, exp := range batch {
            // Forward pass
            action, value := m.Forward(exp.Observation)
            ...
            // Update weights
            for i := range m.actorWeights {
                m.actorWeights[i] -= m.learningRate * float32(exp.Action[i%3]) * advantage
            }
            for i := range m.criticWeights {
                m.criticWeights[i] -= m.learningRate * (value - returns)
            }
        }
    }
...
func (s *TrainingServer) handleGetAction(c *gin.Context) {
    startTime := time.Now()
    s.logger.Log("info", "[mario] Starting get_action request processing")
    ...
    if err := c.BindJSON(&req); err != nil {
        s.logger.Log("error", fmt.Sprintf("[mario] JSON parsing failed: %v", err))
        c.JSON(http.StatusBadRequest, gin.H{"error": "Invalid request"})
        return
    }
    s.logger.Log("info", fmt.Sprintf("[mario] JSON parsing took: %v", time.Since(jsonStart)))
    
    // Store instance data with player ID
    instanceStart := time.Now()
    s.instanceMutex.Lock()
    if s.instanceData[req.InstanceID] == nil {
        s.instanceData[req.InstanceID] = make(map[string]interface{})
    }
    s.instanceData[req.InstanceID]["player_id"] = req.PlayerID
    s.instanceMutex.Unlock()
    s.logger.Log("info", fmt.Sprintf("[mario] Instance data update took: %v", time.Since(instanceStart)))
    
    // Forward pass
    forwardStart := time.Now()
    action, value := s.model.Forward(req.Observation)
    s.logger.Log("info", fmt.Sprintf("[mario] Forward pass took: %v", time.Since(forwardStart)))
    ...
    s.logger.Log("info", fmt.Sprintf("[mario] Total get_action processing time: %v", time.Since(startTime)))
    s.logger.Log("info", fmt.Sprintf("[mario] Action response for player %d: %v", req.PlayerID, response))
